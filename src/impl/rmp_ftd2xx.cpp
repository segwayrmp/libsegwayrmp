...
FTD2XXRMPIO::FTD2XXRMPIO() : configured(false), config_type(by_none), baudrate(460800), is_open(false) {
...
void FTD2XXRMPIO::connect() {
    if(!this->configured) {
        throw(ConnectionFailedException("The usb port must be configured before connecting."));
    }
    
    FT_STATUS result;
    
    this->enumerateUSBDevices();
    ...
    if (result != FT_OK) {
        throw(ConnectionFailedException(this->getErrorMessageByFT_STATUS(result, "setting the baudrate").c_str()));
    }
    ...
    if (result != FT_OK) {
        throw(ConnectionFailedException(this->getErrorMessageByFT_STATUS(result, "setting flowcontrol").c_str()));
    }
    ...
    if (result != FT_OK) {
        throw(ConnectionFailedException(this->getErrorMessageByFT_STATUS(result, "purging the buffers").c_str()));
    }
...
int FTD2XXRMPIO::read(unsigned char* buffer, int size) {
    ...
    if (result != FT_OK) {
        throw(ReadFailedException(this->getErrorMessageByFT_STATUS(result, "reading").c_str()));
    }
...
void FTD2XXRMPIO::enumerateUSBDevices() {
    FT_STATUS result;
    ...
    if (result != FT_OK) {
        throw(ReadFailedException(this->getErrorMessageByFT_STATUS(result, "setting vid and pid").c_str()));
    }
    #endif
    ...
    if (result != FT_OK) {
        throw(ReadFailedException(this->getErrorMessageByFT_STATUS(result, "enumerating devices").c_str()));
    }
    ...
    if (number_of_devices > 0) {
        device_info = (FT_DEVICE_LIST_INFO_NODE*)malloc(sizeof(FT_DEVICE_LIST_INFO_NODE)*number_of_devices);
        try {
...
void FTD2XXRMPIO::configureUSBBySerial(std::string serial_number, int baudrate) {
...
void FTD2XXRMPIO::configureUSBByDescription(std::string description, int baudrate) {
...
void FTD2XXRMPIO::configureUSBByIndex(unsigned int device_index, int baudrate) {
...
std::string FTD2XXRMPIO::getErrorMessageByFT_STATUS(FT_STATUS result, std::string what) {
